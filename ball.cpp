...
void ball::Update(sf::RenderWindow* window)
{
	...
	if (this->checkCollision(this->_player1) || this->checkCollision(this->_player2)) // neu ball va cham voi _player1, _player2
	{
		n -= 0.02f; // giam n di 0.02
		this->_velocity.x *= n; // nhan _velocity.x voi n
	}

	// neu ball va cham voi thanh` tren hoac duoi
	if (this->getPosition().y < 0 || this->getPosition().y + this->getGlobalBounds().height > window->getSize().y)
	{
		this->_velocity.y *= -1; // nhan _velocity.y voi n
	}
...
{
	this->_player1->setPosition(0, (window->getSize().y / 2) - 50); // dat vi tri player1 ve giua thanh` trai
