...
void main_game::Initialize(sf::RenderWindow* window)
{
	...
	this->_life->setPosition((window->getSize().x / 2) - (this->_life->getGlobalBounds().width) / 0.5 /*window->getSize().x - (this->_life->getGlobalBounds().width) - 10*/, 0);
	...
	//this->_lives = new sf::Text("Lives:", *_font, 32U);
	//this->_lives->setOrigin(this->_lives->getGlobalBounds().width / 2, this->_lives->getGlobalBounds().height / 2);
	//this->_lives->setPosition(this->_life->getPosition().x - 50, 15);

	// khoi tao _ball la new ball voi tham so _score1, _score2, _player1 va _player2
	this->_ball = new ball(this->_life, /*this->_score2,*/ this->_player1/*, this->_player2*/);
...
void main_game::Update(sf::RenderWindow* window)
{
	...
	this->_life->Update(); // goi ham Update cua _score1 voi tham so window
	
	for (int i = 0; i < 12; i++)
	{
		this->_brick[i].Update();
	}
